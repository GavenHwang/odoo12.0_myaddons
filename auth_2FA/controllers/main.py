                    ...
                    if valid:
                        if otp_first_use:
                            values['QRCode'] = 'data:image/png;base64,' + request.env['res.users'].browse(
                                user_id).otp_qrcode
                            values['text'] = _('You are the first time to use OTP, please scan the QRCode to get validation code.you should store this QRCode image and take good care of it! ')
